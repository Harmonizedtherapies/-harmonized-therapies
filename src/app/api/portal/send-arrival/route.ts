...
<tr><td style="padding:40px 40px 36px;">
  ...
  <p style="margin:0 0 32px;font-size:16px;line-height:1.8;color:#555;">I'm looking forward to seeing you today. Before your session, take a moment to check in with how you're arriving — it only takes a minute.</p>
  <table cellpadding="0" cellspacing="0" style="margin-bottom:36px;">
    <tr><td style="background:#2D5A3D;border-radius:100px;">
      <a href="${link}" style="display:inline-block;padding:14px 28px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#fff;text-decoration:none;">Open check-in</a>
    </td></tr>
  </table>
...
export async function POST(req: NextRequest) {
  ...
  const { data: session } = await supabase
    .from('sessions')
    .select('arrival_token, clients(name, email)')
    .eq('id', session_id)
    .single()
  ...
  const link = `${SITE_URL}/arrival?token=${session.arrival_token}`

  const { error } = await getResend().emails.send({
    from: FROM,
    to: client.email,
    subject: 'Before your session today — Harmonized Therapies',
