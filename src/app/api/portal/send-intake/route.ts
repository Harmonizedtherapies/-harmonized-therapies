...
<tr><td style="padding:40px 40px 36px;">
  ...
  <p style="margin:0 0 32px;font-size:16px;line-height:1.8;color:#555;">Before your first session, I'd love to know a little more about you. When you have a quiet moment, please fill in your intake form — it'll only take a few minutes.</p>
  <table cellpadding="0" cellspacing="0" style="margin-bottom:36px;">
    <tr><td style="background:#2D5A3D;border-radius:100px;">
      <a href="${link}" style="display:inline-block;padding:14px 28px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#fff;text-decoration:none;">Complete intake form</a>
    </td></tr>
  </table>
...
export async function POST(req: NextRequest) {
  ...
  const { data: client } = await supabase.from('clients').select('name, email, intake_token').eq('id', client_id).single()
  if (!client?.email) return NextResponse.json({ error: 'No email on file' }, { status: 400 })

  const firstName = client.name.split(' ')[0]
  const link = `${SITE_URL}/welcome?token=${client.intake_token}`

  const { error } = await getResend().emails.send({
    from: FROM,
    to: client.email,
    subject: 'Your intake form — Harmonized Therapies',
